...
module "records" {
  ...
  records = [
    {
      name = "roboshop-${var.environment}" # roboshop-dev.awsdevopsjourney.online
      type = "A"
      alias = {
        name    = module.ingres_alb.dns_name
        zone_id = module.ingres_alb.zone_id  # This belongs ALB internal hosted zone, not ours
      }
...
resource "aws_lb_listener_rule" "static" {
  ...
  action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.roboshop_tg.arn
  }
  condition {
    host_header {
      values = ["roboshop-${var.environment}.${var.zone_name}"] #roboshop-dev.awsdevopsjourney.online
    }
  }
}
